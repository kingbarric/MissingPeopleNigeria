...
@IonicPage()
@Component({
  selector: 'page-search-by-photo',
  templateUrl: 'search-by-photo.html',
})
export class SearchByPhotoPage {
  ...
  search(){
    ...
    if(this.imageData !==''){
      ...
      this.crudService.saveData('reports/searchface',reports,0)
      .subscribe((e:any)=>{
        
        console.log('data saved: ',e);
        if(e){
         this.crudService.toast('Search completed');
         this.reports = e;
         //this.navCtrl.setRoot(HomePage);
         this.loading = false;
       }else{
        this.crudService.toast('No Result found! for this image');
       }
      })
    }else{
